...
Your job: identify REPEATED workflows (a sequence of domains that occurred 2+ times). For each, output a strict JSON object matching the schema.
...
For "ai_leverage": for each step, classify whether the step is "automatable" (strong LLM task — email parsing, classification, summarization, structured extraction), "deterministic" (a simple script or API call beats an LLM), or "judgment" (requires human discretion). Include a one-sentence "why".
